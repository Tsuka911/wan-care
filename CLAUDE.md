...
### 誤タップ防止
- 「実施済み」ボタンは確認ダイアログ付き（フォーム開閉でカード位置が動き、下の予定の実施済みを誤タップする問題があったため）
...
- もうすぐの予定：`renderUpcoming()`
- 体重折れ線グラフ：`renderWeightLine()` → `id="weightLineCard"`
- 散歩サマリー（東海道の旅含む）：`renderWalkSummary()` → `id="walkSummaryCard"`
- 思い出振り返り：`renderMemoryCard()` → `id="memoryCard"`
...
- 各カードに2つのボタン：
  ...
  - チェックマーク → `markDone()`（確認ダイアログ後にnextをクリア）
